...
public abstract class Message {
  ...
  public byte[] toArray() {
    ...
    rawBytes[i++] = START_MESSAGE_MARK;
    rawBytes[i++] = getId();
    rawBytes[i++] = getType();
    rawBytes[i++] = getFlags();
    rawBytes[i++] = getPayloadLength();
    for (Byte b : getPayload()) {
      rawBytes[i++] = b;
    }
    rawBytes[i++] = END_MESSAGE_MARK;
    return rawBytes;
  }
  ...
  public String toString() {
    return "Message{" +
        "id=" + id +
        ", type=" + getTypeName() +
        ", flags=" + getFlagsNames() + " (" + String.format("0x%02x", flags) + ")" +
